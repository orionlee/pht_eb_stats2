...
def _get_subject_ids_of_tag_page(tag, page, end_subject_id_exclusive=None, json=False):
    url = f"https://talk.zooniverse.org/tags/popular?http_cache=true&taggable_type=Subject&section=project-7929&name={tag}&page={page}"
    res = fetch_json(url)
    if json:
        return res
...
if __name__ =="__main__":
    ...
    # return get_subject_ids_of_tag("eclipsingbinary", 1, 3230, end_subject_id_exclusive=68601250, page_result_func=save_subject_ids_of_page)
    ...
    get_subject_ids_of_tag("eclipsingbinary", page_start, page_end_inclusive, end_subject_id_exclusive=end_subject_id_exclusive, page_result_func=save_subject_ids_of_page)
